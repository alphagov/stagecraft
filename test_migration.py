...
# for filename, json in spotlight_json(os.path.join(os.path.dirname(__file__), 'test_import')):
for filename, json in spotlight_json(os.path.join(os.path.dirname(__file__), '../spotlight/app/support/stagecraft_stub/responses')):
    ...
    if not isinstance(json, dict):
        ...
        logger.warning(
            'skipping dashboard {} as it does not have a published attribute'.format(
                filename))
    else:
